...
将以下参数封装成json字符串contextPlain，然后使用 商户平台公钥进行加密，得到密文context，最后使用GOW平台私钥生成签名。
最终向商户平台发起请求，请求参数为 
...
| 参数名称      | 说明                            |    schema       |
| ------------ | ------------------------------|---------------------- |
| code         | 000000 已受理成功；000001流水号重复 ；000002受理失败 ； 000003参数错误；000004 解密失败； 000012签名错误 ；  150001商户不存在  |       |
